...
/* EV_REL sub-codes: pointer motion vs. scroll wheel are separable */
#define REL_X      0x00
#define REL_Y      0x01
#define REL_HWHEEL 0x06
#define REL_WHEEL  0x08

/* EV_KEY codes the remote emits to show/hide the pointer cursor */
#define KEY_CURSOR_SHOW 1198
#define KEY_CURSOR_HIDE 1199

/*
 * Pointer/scroll suppression is OFF by default and controlled at runtime by
 * two flags in keybinds.json:
 *     "disable_pointer": true   -> drop cursor motion (REL_X / REL_Y)
 *     "disable_scroll":  true   -> drop scroll wheel  (REL_WHEEL / REL_HWHEEL)
 * With neither set (the default after a fresh install), nothing is suppressed.
 */
...
static int       g_disable_pointer = 0;   /* runtime flag from config */
static int       g_disable_scroll  = 0;   /* runtime flag from config */
...
/* Read a JSON boolean-ish value ("true"/"false" or 1/0). Returns def if absent. */
static int find_bool(const char *json, const char *key, int def) {
    char search[64]; int n = 0;
    search[n++] = '"';
    for (int i = 0; key[i] && n < 62; i++) search[n++] = key[i];
    search[n++] = '"'; search[n] = '\0';
    ...
    p += n;
    while (*p == ' ' || *p == ':') p++;
    if (*p == 't' || *p == '1') return 1;   /* true / 1 */
    if (*p == 'f' || *p == '0') return 0;    /* false / 0 */
    return def;
}
...
static void reload_config(void) {
    ...
    /* top-level runtime flags (default off) */
    g_disable_pointer = find_bool(buf, "disable_pointer", 0);
    g_disable_scroll  = find_bool(buf, "disable_scroll", 0);
    log_int("[hook] disable_pointer=", g_disable_pointer);
    log_int("[hook] disable_scroll=", g_disable_scroll);
...
ssize_t write(int fd, const void *buf, size_t count) {
    ...
    if (ev->type == EV_REL) {
        if (g_disable_pointer && (ev->code == REL_X || ev->code == REL_Y))
            return (ssize_t)count;   /* swallow cursor motion */
        if (g_disable_scroll && (ev->code == REL_WHEEL || ev->code == REL_HWHEEL))
            return (ssize_t)count;   /* swallow scroll wheel */
    }

    /* When the pointer is disabled, also swallow the show/hide cursor events so
       the cursor never appears (not just stops moving). */
    if (ev->type == EV_KEY && g_disable_pointer &&
...
static void hook_init(void) {
    ...
    hook_log("[hook] pointer/scroll suppression is config-driven (default off)");
    reload_config();
}
